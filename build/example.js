...
`],posts=[],authors=[];const resolvers={Query:{posts:()=>posts,authors:()=>authors},Mutation:{upvotePost:(b,{postId:c})=>{const d=posts.filter(e=>c===e.id);if(!d)throw`Couldn't find post with id ${c}`;return d.votes+=1,d},upvoteAuthor:(b,{authorId:c})=>{const d=authors.filter(e=>c===e.id);if(!d)throw`Couldn't find author with id ${c}`;return d.votes++,d},addPost:(b,{title:c,votes:d,author:e})=>{let f={id:posts.length,title:c,votes:d,author:e};return posts.push(f),f},addAuthor:(b,{firstName:c,lastName:d,votes:e})=>{let f={id:authors.length,firstName:c,lastName:d,votes:e};return authors.push(f),f}},Post:{author:b=>authors.filter(c=>c.id===b.author).shift()}};let s=__webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0__index__["default"])(schema,resolvers);const log=b=>console.log(JSON.stringify(b,null,2));queries.reduce((b,c)=>b.then(()=>s(c).then(log)),Promise.resolve(!0));
